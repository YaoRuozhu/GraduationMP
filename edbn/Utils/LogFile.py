...
class LogFile:
    ...
    def split_percentages(self, percentages):
        periods_logfile = []
        rest_logfile = self
        for i in range(len(percentages)):
            percentage = percentages[i]
            print(percentage/100)
            period_logfile, rest_logfile = rest_logfile.splitTrainTest(percentage, split_case=True, method="train-test")
            rest_logfile.data.reset_index(drop=True, inplace=True)
            rest_logfile.contextdata.reset_index(drop=True, inplace=True)
            #print(period_logfile.data.shape[0])
            print('period_logfile'+str(i),period_logfile.data.shape[0])
            periods_logfile.append(period_logfile)
        period_logfile = rest_logfile
        periods_logfile.append(period_logfile)
        #print('period_logfile'+str(i),period_logfile.data.shape[0])
        return periods_logfile
