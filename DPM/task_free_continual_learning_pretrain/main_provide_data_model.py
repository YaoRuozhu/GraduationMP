...
def experiment(data, learning_object, tags=['Online Continual','Online Continual No Hardbuffer','Online','Online No Hardbuffer']):
    ...
    for tag in tags:
        ...
        training_losses, loss_window_means[tag], update_tags[tag], loss_window_variances[tag], test_losses[tag],future_losses, prediction_results = results
  
    if False:
        legend=[]
        plt.title('training accuracy')
        #for i in range(ntasks): plt.axvline(x=(i+1)*z,color='gray')
        for tag in tags:    
    ...
    tag='Online Continual'
    return training_losses, test_losses, future_losses, loss_window_means[tag], update_tags[tag], loss_window_variances[tag], prediction_results


def main(file,recent_buffer_size,hard_buffer_size):
    ...
    basic_model = m.train(d.train,{"epochs": 20, "early_stop": 10})
    ...
    training_losses, test_losses, future_losses,loss_window_means, update_tags, loss_window_variances, prediction_results = experiment(data_sampler, learning_object, tags)
    ...
    return training_losses, test_losses, future_losses, running_time, loss_window_means, update_tags, loss_window_variances, prediction_results
