...
class Method:
    ...
    def test_and_update_indices(self, model, data, evWindow, prevTestInd, currentTestInd, reset):
        ...
        print('train data:','evWindow[1]: ', evWindow[1], 'evWindow[0]: ', evWindow[0],'test data:','prevTestInd: ', prevTestInd, 'currentTestInd: ', currentTestInd)
        # if reset:
        #     model = self.train(train_data)
        # else:
        #     model = self.update(model, train_data)
        # batch_inputs,batch_expected = self.update(model, train_data)
