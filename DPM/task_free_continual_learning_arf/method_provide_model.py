...
class Task_free_continual_learning_provide_model():
    ...
    def __init__(self,
        ...
        #     tf.random.normal(var.shape, mean=0.0, stddev=0.1, dtype=tf.float32)
    ...
    def method(self,
        ...
        for t in range(self.ntasks):
            for s in range(len(data.inputs[t])):
                ...
                #print('inputs',data.inputs[t][s],'labels',data.labels[t][s],'rb',recent_buffer)
                #print(len(recent_buffer))
                ...
                if len(recent_buffer) == self.recent_buffer_size:
                    ...
                    for gs in range(self.gradient_steps):
                        with tf.GradientTape() as tape:
                            ...
                            y_sup = tf.one_hot(tf.convert_to_tensor(np.asarray(y).reshape(-1, 1)), depth=2, dtype=tf.float32)
                            recent_loss = self.loss_fn(y_sup, y_pred)
                            total_loss = tf.reduce_sum(self.loss_fn(y_sup, y_pred))

                            if use_hard_buffer and len(hard_buffer) != 0:
                                # evaluate hard buffer
                                yh_pred = self.model(tf.convert_to_tensor(np.asarray(xh).reshape(-1, self.dim), dtype=tf.float32))
                                yh_sup = tf.one_hot(tf.convert_to_tensor(np.asarray(yh).reshape(-1, 1)), depth=2, dtype=tf.float32)

                                hard_loss = self.loss_fn(yh_sup, yh_pred)
                                total_loss += tf.reduce_sum(self.loss_fn(yh_sup, yh_pred))
                            ...
                            if continual_learning and len(star_variables) != 0 and len(omegas) != 0:
                                for pindex, p in enumerate(self.model.trainable_variables):
                                    total_loss += self.MAS_weight / 2. * tf.reduce_sum(
                                        tf.convert_to_tensor(omegas[pindex], dtype=tf.float32) * (p - star_variables[pindex]) ** 2)
                        ...
                        if use_hard_buffer and len(hard_buffer) != 0:
                            ...
                            xt = x[:]
                            yt = y[:]
                        yt_pred = model(tf.convert_to_tensor(np.asarray(xt).reshape(-1, self.dim), dtype=tf.float32))
                        accuracy = np.mean(np.argmax(yt_pred.numpy(), axis=1) == yt)
